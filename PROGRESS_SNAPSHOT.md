...
- **TG 协作群 = `acc_work_gp`**(2026-05-04 Jonathan 拉 4 Bot 进群,chat_id 待抽)
- `report_parser.parse_report()` 纯函数 + 11 单测全过(本地 + 服务器双跑)
- fixture `meta_ops/facilitator/tests/fixtures/2026W19.md`(真实周报,4 条决策)

**Step 4 启动前置依赖**:`.env` `TG_ADMIN_CHAT_ID` 当前空,需用 acc_work_gp 的 chat_id 填(/start handler 已加日志,Jonathan 在群里 /start@acc_ana_bot 即可抽出)。
...
| TG 协作群 `acc_work_gp` chat_id | 待 Jonathan 在群里 `/start@acc_ana_bot` 触发,从 journal log 抽 chat.id 后填 .env `TG_ADMIN_CHAT_ID` |
